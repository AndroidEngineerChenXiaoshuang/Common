...
import com.jixingmao.common.base.BaseView;
...
public abstract class DefaultObserver<T extends BasicResponse<D>, D> implements Observer<T> {

    public static final int HTTP_SUCCESS_CODE = 200;

    private IView mIView;
    private boolean mIsShowDefaultView;

    public DefaultObserver(IView iView, boolean isShowDefaultView) {
        mIView = iView;
        mIsShowDefaultView = isShowDefaultView;
    }
    ...
    public void onError(Throwable e) {
        ...
        if (mIsShowDefaultView) {
            if (NetWorkUtils.isAvailable()) {
                mIView.showNetWorkErrorView();
            } else {
                mIView.showNoNetWorkErrorView();
            }
        }
